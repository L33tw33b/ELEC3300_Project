...
void DrawPiano() {
    const uint16_t whiteColor = 0xFFFF; // White color for the keys
    const uint16_t blackColor = 0x0000; // Black color for the keys
    const uint16_t textColor = 0xFFFF; // White color for the text
    const uint16_t textBgColor = 0x0000; // Black background for the text

    //Displ_Init(Displ_Orientat_0);
    //Displ_CLS(blackColor); // Clear screen with black color

    int16_t keyWidth = 20; // Width of each white key
    int16_t whiteKeyHeight = 80; // Height of white keys
    int16_t blackKeyHeight = 50; // Height of black keys
    int16_t totalWhiteKeys = 7; // Number of white keys
    int16_t totalBlackKeys = 5; // Number of black keys
    int16_t cornerRadius = 2; // Corner radius for round rect

    int16_t displayWidth = 240; // Display width
    int16_t displayHeight = 320; // Display height

    // Calculate the total width of the piano (only white keys are considered for full width)
    int16_t pianoWidth = totalWhiteKeys * keyWidth;
    int16_t startX = (displayWidth - pianoWidth) / 2;
    int16_t startY = (displayHeight - whiteKeyHeight) / 2 - 40; // Center vertically and adjust

    // Draw white keys
    for (int i = 0; i < totalWhiteKeys; i++) {
        Displ_fillRoundRect(startX + i * keyWidth, startY, keyWidth - 1, whiteKeyHeight, cornerRadius, whiteColor);
        Displ_drawRoundRect(startX + i * keyWidth, startY, keyWidth - 1, whiteKeyHeight, cornerRadius, blackColor);
    }

    // Draw black keys
    for (int i = 0; i < totalBlackKeys; i++) {
        int16_t blackKeyX = startX + keyWidth * (1 + i) - keyWidth / 4;
        if (i >= 2) blackKeyX += keyWidth / 2; // Adjust spacing after 3 black keys
        Displ_fillRoundRect(blackKeyX, startY, keyWidth / 2, blackKeyHeight, cornerRadius, blackColor);
    }

    sFONT font = Font16; // Font settings
    uint8_t textSize = 1; // Font size
    int16_t textY = startY + whiteKeyHeight + 10; // Below the piano
    int16_t textHeight = 20; // Text height

    // Correctly calculate text area to ensure centering
    Displ_CString(startX, textY, startX + pianoWidth, textY + textHeight, "Synthesizer", font, textSize, textColor, textBgColor);
}
